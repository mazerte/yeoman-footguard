...
Generator.prototype.createHelperFiles = function createCollectionFiles() {
	...
	if( this.test ) {
		this.template('helper_spec.coffee', path.join('src/coffee/spec/helpers', this.folder, this.name + '_helper_spec.coffee'));
	  ...
	  body = generatorUtil.rewrite({
	    ...
	    haystack: body,
	    splicable: [
	      '	"' + path.join('spec/helpers/', this.folder, this.name + '_helper_spec') + '"'
	    ]
	  });
