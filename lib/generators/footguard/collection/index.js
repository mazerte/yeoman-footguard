...
Generator.prototype.createCollectionFiles = function createCollectionFiles() {
	...
	if( this.test ) {
		this.template('collection_spec.coffee', path.join('src/coffee/spec/collections', this.folder, this.name + '_collection_spec.coffee'));
	  ...
	  body = generatorUtil.rewrite({
	    ...
	    haystack: body,
	    splicable: [
	      '	"' + path.join('spec/collections', this.folder, this.name + '_collection_spec') + '"'
	    ]
	  });
