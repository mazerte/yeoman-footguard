...
Generator.prototype.createModelFiles = function createCollectionFiles() {
	...
	if( this.test ) {
		this.template('model_spec.coffee', path.join('src/coffee/spec/models', this.folder, this.name + '_model_spec.coffee'));
	  ...
	  body = generatorUtil.rewrite({
	    ...
	    haystack: body,
	    splicable: [
	      '	"' + path.join('spec/models/', this.folder, this.name + '_model_spec') + '"'
	    ]
	  });
