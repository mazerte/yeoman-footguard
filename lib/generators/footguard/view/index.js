...
Generator.prototype.createViewFiles = function createCollectionFiles() {
	...
	if( this.test ) {
		this.template('view_spec.coffee', path.join('src/coffee/spec/views', this.folder, this.name + '_view_spec.coffee'));
	  ...
	  body = generatorUtil.rewrite({
	    ...
	    haystack: body,
	    splicable: [
	      '	"' + path.join('spec/views/', this.folder, this.name + '_view_spec') + '"'
	    ]
	  });
